package com.amazonaws.lambda.admin_operation_handler.DBtrasaction;
...
public class DBOperation {
	...
	public static List<ItemS3Details> getILCTimesheetDetails(Item item, LambdaLogger logger) throws SQLException {
		...
		try {
			...
			String query = null;
			// DB Query for ILC
			query = "SELECT s3_url, upload_datetime, status FROM internal_app.ilc_timesheet_details WHERE" + " employee_id= '"
					+ item.getEmployeeId() + "' AND " + "employee_name='" + item.getEmployeeName()
					+ "' AND status=0 AND item_type= '" + item.getItemType() + "'";
			...
			if (resultSet != null) {
				while (resultSet.next()) {
					...
					itemS3Details.setDate(LocalDate.fromDateFields(resultSet.getDate("upload_datetime")));
					itemS3Details.setStatus(resultSet.getBoolean("status"));
