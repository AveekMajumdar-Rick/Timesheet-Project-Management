package com.amazonaws.lambda.ilc_timesheet_view.utils;
		...
		if (!CollectionUtils.isNullOrEmpty(itemS3Details)) {
			itemS3Details.stream().forEach(items -> {
				...
				try {
					...
					response.put("comments", items.getComments());
					response.put("status",
							items.getStatus() == 2 ? "Rejected" : items.getStatus() == 1 ? "Approved" : "Pending");
